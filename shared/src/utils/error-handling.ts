    ...
    switch (error.status) {
      case 400:
        ...
        if (hasData(error) && error.data?.data) {
          ...
          // Only classify as authentication error if:
          // 1. Error code explicitly indicates invalid credentials
          // 2. Error message explicitly indicates authentication failure (not validation)
          // Password validation errors during signup should be treated as validation errors
          const isAuthFailure =
          ...
          // All other 400 errors are validation errors
          return {
            type: 'validation',
