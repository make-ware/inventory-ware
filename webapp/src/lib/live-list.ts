/**
 * Pure merge helpers that fold PocketBase realtime (SSE) events into a cached
 * TanStack `InfiniteData<ListResult<T>>` — the generic core of the "paged list
 * that stays live" pattern (consumed by `@/hooks/use-live-infinite-list`).
 *
 * Contract — every invariant here exists to keep the cache honest without
 * refetching:
 *
 * - Every function returns the SAME reference when the event is a no-op (echo
 *   of a local write, stale out-of-order event, irrelevant record), so
 *   TanStack Query's structural sharing skips observer notifications entirely.
 ...
 * - Untouched page-item arrays keep their references; page envelopes are only
 *   rewritten when their totals actually change.
 *
 * Sort-window rule for records not in the loaded window: insert only when the
 * record sorts strictly before the last loaded item (it belongs inside the
 * window) or the window is complete (no more pages server-side). A `create`
 * that lands beyond the window still bumps `totalItems` so "Showing X of Y"
 * stays honest; an `update` for an unknown id beyond the window is ignored
 * entirely — the record already exists server-side and is already counted, so
 * load-more or the next refetch picks it up.
 *
 * Nothing in this module touches PocketBase, React or the query client: it is
 * data in, data out, which is what makes the rules above testable.
 */
...
export interface LiveListSpec<T extends LiveListRecord> {
  ...
  /**
   * Mirror of the server sort. MUST embed a total tiebreak (primary key, then
   * e.g. created desc, then id) so distinct records never compare equal — a 0
   * result is how "sort key unchanged" is detected on updates.
   */
  compare: (a: T, b: T) => number;
  /**
   * False when `compare` cannot read its key off this record (e.g. the sort
   * key lives on an expand the SSE event did not carry). Such records are
   * never (re)positioned: known ids are replaced in place, unknown ids are not
   * inserted.
   */
 ...
 * `updated` stamps ("2026-08-25 10:00:00.123Z") compare correctly as strings;
 * equal stamps mean the cache already holds this write (an echo), so only
 * strictly-newer records apply. Missing stamps fail open — applying an
 * identical record is idempotent, just not reference-stable.
 */
 ...
 */
export function rewriteTotals<T extends LiveListRecord>(
  pages: ListResult<T>[],
  totalItems: number
): ListResult<T>[] {
 ...
 * are render windows, and a page holding 13 rows is a smaller lie than a row
 * that silently vanishes off the end of page 1.
 */
...
export function applyListEvent<T extends LiveListRecord>(
  ...
  if (action === 'delete') {
    ...
    // Deleted from an unloaded page: still shrink the server total (PB delete
    // events carry the full record, so `matches` is evaluable).
  ...
  if (at) {
    ...
    // Same id ⇒ the total tiebreak is 0 ⇒ a 0 means the sort key is unchanged;
    // keep position. Also keep position when the record cannot be compared
    // (position may be stale until the next refetch).
    ...
    // reinsertion beyond an incomplete window drops the record from the window
    // (still counted server-side; load-more re-encounters it and dedupeById
    // guards the overlap).
    return insertSorted(removeAt(data, at, 0), record, spec, 0).data;
  }

  // Unknown id. Without a comparable sort key the record cannot be placed: a
  // create still bumps totals, an update is already counted server-side.
  if (!comparable) {
  ...
  // Update for an id we never loaded (missed create or unloaded page): insert
  // only within the window; beyond it, ignore — no totals bump, since the
  // record is already included in the server's count.
  const { data: next, inserted } = insertSorted(data, record, spec, 0);
  return inserted ? next : data;
}

/**
 * The single-envelope form of `applyListEvent`, for lists cached as one
 * `ListResult` rather than as infinite pages (the images library — see
 * `@/hooks/use-images`). One page is just an infinite list with one page, so
 * this shares every rule above rather than reimplementing them.
 */
export function applyPageEvent<T extends LiveListRecord>(
  page: ListResult<T>,
  action: RealtimeAction,
  record: T,
  spec: LiveListSpec<T>
): ListResult<T> {
  const data: LiveListData<T> = { pages: [page], pageParams: [page.page] };
  return applyListEvent(data, action, record, spec).pages[0];
}
 ...
 * (realtime-merged, newest) occurrence wins. Same reference when there are no
 * duplicates.
 */
