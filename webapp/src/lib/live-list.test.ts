...
/**
 * Ported from video-ware's `utils/__tests__/live-list.test.ts` along with the
 * module itself, so the invariants the two apps share are pinned by the same
 * cases: same-reference no-ops, the strictly-newer `updated` stamp, untouched
 * page arrays keeping their identity, and the sort-window rule.
 */
...
describe('isRecordNewer', () => {
  it('accepts a strictly newer stamp and rejects an equal or older one', () => {
    expect(isRecordNewer({ updated: T2 }, { updated: T1 })).toBe(true);
    expect(isRecordNewer({ updated: T1 }, { updated: T1 })).toBe(false);
    expect(isRecordNewer({ updated: T1 }, { updated: T2 })).toBe(false);
  });

  it('fails open when either stamp is missing', () => {
    expect(isRecordNewer({}, { updated: T1 })).toBe(true);
    expect(isRecordNewer({ updated: T1 }, {})).toBe(true);
  });
});

describe('applyPageEvent', () => {
  const page = (items: Item[], totalItems?: number): ListResult<Item> =>
    makeData([items], { perPage: 100, totalItems }).pages[0];

  it('merges into a single envelope and keeps its reference on a no-op', () => {
    const start = page([makeItem('a', created(9))]);
    const echo = makeItem('a', created(9));
    expect(applyPageEvent(start, 'update', echo, spec)).toBe(start);
  });

  it('inserts a create in sort order and bumps the total', () => {
    const start = page([makeItem('b', created(8))]);
    const record = makeItem('a', created(9), { updated: T1 });
    const result = applyPageEvent(start, 'create', record, spec);
    expect(result.items.map((i) => i.id)).toEqual(['a', 'b']);
    expect(result.totalItems).toBe(2);
  });

  it('removes a delete and shrinks the total', () => {
    const start = page([makeItem('a', created(9)), makeItem('b', created(8))]);
    const result = applyPageEvent(
      start,
      'delete',
      makeItem('a', created(9)),
      spec
    );
